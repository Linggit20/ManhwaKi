...
const PopularWebtoon = () => {
  ...
  return (
    <section className='popular-webtoons text-t-white bg-bg-200 rounded-md'>
      <div className='px-4 py-2 mb-3 border-bg-100 border-b'>
        <h1 className='font-bold'>Popular</h1>
      </div>
      <div className='px-4'>
        <div className='grid grid-cols-3 gap-4 px-2 bg-bg-100 py-2 text-sm rounded-md mb-3'>
      ...
      {slicedWebtoonList.map((webtoon, index) => (
        <div key={index} className='flex items-center mb-3 border-bg-100 border-b px-4 pb-3'>
          <span className='border px-2 text-[12px] rounded-sm  mr-3'>{index + 1}</span>
          <div className='self-start overflow-hidden w-[35%] xl:w-[26%]'>
            <img
          ...
          <div className='self-start flex-1'>
            <h2 className='font-medium mb-1 transition-colors duration-300 hover:text-primary cursor-pointer'>{webtoon.title}</h2>
